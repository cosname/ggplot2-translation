options(digits = 2)
## 图10.1
# 当不断放大图形时，交互地使用 last_plot() 可以快速有效地找到最佳视图。最后
# 一张图上添加了一条截距为 0 斜率为 1 的直线。图中可以看到数据中没有正方形的钻石。
...
## 图10.3
# 使用“静默”的 x 和 y 标度时，移除了标签并隐藏了刻度和网格线。
...
## 图10.4
# 创建一个自定义的几何形状函数，用类似 (但不相同) 的组件创建图形时即可调用。
