...
## 图2.3
# 将 alpha 值从 1/10(左) 变动到 1/100(中) 再到 1/200(右)，来看大部分的点在哪里
# 进行重叠。
...
## 图2.6 
# 在运用广义可加模型作为平滑器时 formula 参数的作用。左图是 formula=y~s(
# x)，右图是 formula=y~s(x,bs="cs")。
library(mgcv)
...
## 图2.7
# 在运用线性模型作为平滑器时 formula 参数的作用。左图是 formula = y ~ x 的默认值，
# 右图是 formula = y ~ ns(x, 5)。
library(splines)
...
## 图2.9
# 改变 alpha 的取值，从左到右分别为 1/5，1/50 和 1/200。随着不透明度的降低，
...
## 图2.10
# 展示钻石重量的分布。左图使用的是 geom="histogram" 右图使用的是 geom="
# density"。
...
## 图2.11
# 变动直方图的组距可以显示出有意思的模式。从左到右，组距分别为 1，0.1 和
# 0.01。只有重量在 0 到 3 克拉之间的钻石显示在图中。
...
## 图2.13 
# 钻石颜色的条形图。左图显示的是分组的计数，右图是按 weight=carat 进行加
# 权，展示了每种颜色的钻石的总重量。
...
# 图可以使得比较不同组的分布时不会受该组样本量大小的影响。高质量的钻石 (颜色 D) 在小
