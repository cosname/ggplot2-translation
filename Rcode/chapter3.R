## 图3.1
# 发动机排量(以升为单位 displ)对高速公路耗油量(英里每加仑 hwy)散点图。点
# 根据汽缸数目着色。该图可以发现影响燃油经济性最重要的因素：发动机排量大
# 小。
qplot(displ, hwy, data = mpg, colour = factor(cyl))

## 图3.2
# 除了用点来表示数据，我们还可以用其他的的几何对象，如直线(左)或条形(右)。
# 对于这个数据，用这两种几何对象所作的图形都没有什么意义，但在语法上是有
# 效的。
# 这里给出了做无图例的图的三种方法：
# 方法1
qplot(displ, hwy, data=mpg, colour=factor(cyl), geom="line") + 
  opts(legend.position = FALSE)
# 方法2
qplot(displ, hwy, data=mpg, colour=factor(cyl), geom="line") +
  guides(color = FALSE)
# 方法3
...
## 图3.3
# 更复杂的图形一般没有特定的名称。这幅图在图3.1的基础上对每个
# 组添加了回归线。这个图应该叫什么名字呢？
qplot(displ, hwy, data=mpg, colour=factor(cyl)) + 
