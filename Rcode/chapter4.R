...
## 图4.1
# 修改图形属性。用factor(cyl)修改颜色(左)，用disp修改y坐标(右)。
...
## 图4.3
# 正确分组时(分组变量group = Subject)每个个体的折线图(左)。错误的分组时连
# 接所有观测点的折线图(右)。此处省略了分组图形属性，效果等同于group = 1。
...
## 图4.7
#一个条形图(左)按组分解后得到的叠加条形图(右)，两者轮廓相同。
...
## 图4.9
# 同一调整(identity adjustment)不适用于条形图(左)，因为后画的条形会挡住先
...
## 图4.10
# 直方图的三种变体。频率多边形(frequency polygon)(左)；散点图，点的大小和
