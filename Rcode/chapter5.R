...
p + geom_text(aes(label = label)) + opts(title = "geom_text")
...
# (左图)。(右图) 对 x 轴进行了放大，xlim = c(55, 70)，并选取了一个更小的组距宽度，
# binwidth = 0.1，较左图揭示出了更多细节。我们可以发现这个分布是轻度右偏的。同时别
...
## 图5.4
# 箱线图可以用于观察针对一个类别型变量 (如 cut) 取条件时 (左图)，或连续型变量
# (如 carat) 取条件时 (右图)，连续型变量的分布情况。对于连续型变量，必须设置 group 图
# 形属性以得到多个箱线图。此处使用了group = round_any(carat, 0.1, floor) 来获得针
# 对变量carat 以 0.1 个单位为大小分箱后的箱线图。
library(plyr)
...
## 图5.5
# 几何对象 jitter 可在二维分布中有一个离散型变量时绘制出一个较为粗略的图
# 形。总体来说，这种数据打散的处理对小数据集更有效。上图展示了 mpg 数据集中离散型变
# 量 class 和连续型变量 city，下图则将连续型变量 city 替换为离散型变量 drv。
...
# 数据本身。左图为变量 depth 的密度图。右图为按照变量 cut 的不同取值上色的版本。
...
## 图5.8
# 以从一个二元正态数据中抽样所得的数据为例，使用 alpha 混合来减轻过度绘制问题。alpha
# 值从左至右分别为：1/3, 1/5, 1/10。
...
## 图5.9
# diamond 数据中的变量table 和变量 depth 组成的图形，展示了如何使用数据打
# 散和 alpha 混合来减轻离散型数据中的过度绘制问题。从左至右为：不加任何处理的点，使用
# 默认扰动参数打散后的点，横向扰动参数为 0:5 (横轴单位距离的一半) 时打散后的点，alpha 值
# 1/10，alpha 值 1/50，alpha 值 1/200。
...
# 栏使用参数bins = 10，右栏使用参数binwidth = c(0.02, 200)。为了节约空间，均略去
# 了图例。
...
d + stat_density2d(geom = "point", aes(size = ..density..),
  contour = F) + scale_area(range = c(0.2, 1.5))
...
## 图5.12
# 函数 borders() 的使用实例。左图展示了美国 (2006 年 1 月) 五十万人口以上的城
# 市，右图为德克萨斯州的城市区划。
library(maps)
data(us.cities)
...
## 图5.14
# 进行数据变换以移除显而易见的效应。左图对 x 轴和 y 轴的数据均取以 10 为底的
...
## 图5.15(书中无代码)
# 展示模型估计结果中变量 color 的不确定性。左图为 color 的边际效应。右图则
# 是针对变量 caret 的不同水平 (level)，变量 color 的条件效应。误差棒显示了 95% 的逐点
# 置信区间。
...
## 图5.16(书中无代码)
# 展示模型估计结果中变量 carat 的不确定性。左图为 caret 的边际效应。右图则是
# 针对变量 color 的不同水平，变量 caret 的条件效应。误差带显示了 95% 的逐点置信区间。
...
## 图5.17
# 函数 stat_summary 的使用示例。首行从左至右分别展示了连续型变量 x 的：中位
# 数曲线，median_hilow() 所得曲线和平滑带，均值曲线，以及 mean_cl_boot() 所得曲线和
# 平滑带。次行从左至右分别展示了离散型变量 x 的：mean() 所得均值点，mean_cl_normal
# () 所得均值点和误差棒，median_hilow() 所得中位数点和值域，以及median_hilow() 所
# 得中位数点和值域条。请注意 ggplot2 展示了整个数据的取值范围，而不仅仅是各种描述性
# 统计量所涉及的范围。
...
## 图5.18
# 使用点的大小来展示权重：无权重 (左图)，以人口数量为权重 (中图)，以面积为权
# 重 (右图)。
...
## 图5.20
# 不含权重信息 (左侧) 以及含权重信息 (右侧) 的直方图。不含权重信息的直方图展
