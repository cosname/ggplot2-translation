...
## 图6.1
# 调整标度默认参数的示例。(左上图) 使用默认标度的图形。(右上图) 手动添加默认
# 标度，并未改变图形外观。(左下图) 调整标度的参数以实现对图例的微调。(右下图) 使用一
...
# 数limits = c(4, 8), breaks = 4:8 的图形，(中栏)breaks = c(5.5,6.5)，(右栏)limits
# = c(5.5,6.5)。
...
## 图6.5
# 个人储蓄率的时间序列图形。(左图) 默认外观，(中图) 每隔 10 年为一断点，以及
# (右图) 使用年月日的格式仅显示在 2004 年内的图形。测量值是在每月月末记录的。
library(scales)
...
## 图6.7
# 温泉喷发时间数据密度估计的三种配色方案。(左图) 默认的颜色梯度，(中图) 自定
# 义的黑白梯度以及 (右图) 中点设为密度均值的 3 点梯度。
...
## 图6.8
# 使用 vcd 包生成的可被良好感知的调色板创建的梯度型颜色标度。从左至右分别为：
# 顺序型、发散型，以及 hcl 热度型调色板。每种标度使用 scale_fill_gradientn 生成，参
# 数 colours 分别设置为 rainbow_hcl(7)、diverge_hcl(7) 和 heat_hcl(7)。
library(vcd)
...
## 图6.9
# 三种 ColorBrewer 调色板，Set1(左图)，Set2(中图) 和 Pastel1(右图)，分别应用于
# 点 (首行) 和条形 (次行)。更明亮的颜色对点效果良好，但对于条形来说过于刺眼。淡色对条
# 形效果良好，但会让点看不清楚。
...
## 图6.10
# 使用手动型标度创建 (左图和中图) 自定义颜色标度和 (右图) 形状标度。
