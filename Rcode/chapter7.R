...
## 图7.6 
# 每种小汽车的城市油耗英里数的点图。(左)车类型按mpg均值排序，
...
## 图7.13
# 坐标系的范围设置 vs 标度的范围设置。(左) 完整的数据集；(中)x 的标度范围设置
# 为 (325,500)；(右) 坐标系 x 轴范围设置为 (325,500)。坐标系的放缩就是图像的放缩，而
...
## 图7.14
# 坐标系的范围设置 vs 标度的范围设置。(左) 完整的数据集；(中)x 的标度范围设置
# 为 (0,2)；(右) 坐标系的 x 轴设置为 (0,2)。比较方块的大小：当设定标度范围时，方块的
...
## 图7.15
# (左) 散点图和相应的平滑曲线，x 轴代表发动机排量 (displ) 和 y 代表城市油耗
# (cty)。(中) 互换两个变量 cty 和 displ 使图形旋转 90 度，平滑曲线拟合的是旋转后的变
# 量。(右) coord_flip 拟合初始数据，然后再翻转输出结果，就变成了以 y 为条件变量刻画 x
# 的曲线了。
...
## 图7.16
# (左) 克拉与价格对数转换后的散点图。直线为回归曲线：log(y)= a + b log(x)。
# (右) 将前面的图转换回去 ( coord_trans(x="pow10",y="pow10"))，标度还原，因此线
# 性趋势变成指数形式，y = k c^x。图形很明显地揭示了克拉越大钻石越贵，且增幅速度越大。
...
## 图7.17
# (左) 堆叠的条状图。(中) 极坐标中堆叠的条状图，常被称为饼图，其中，x 被
# 映射成了半径，y 坐标映射成了角度，coord_polar(theta="y")。(右) 映射的变量相反，
# coord_polar(theta="x")，这个图常被称为牛眼图。
# 堆叠条状图
