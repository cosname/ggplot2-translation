...
## 图9.1
# 图中是不同颜色钻石 price 对 carat 的平滑趋势。左图是全部数据，可以看到重量
# 大于 2 克拉后，标准差就激增，因为重量大于 2 克拉的钻石的个数较少。右图我们只画重量
# 小于 2 克拉的钻石，重点关注数据较多的区域。
...
## 图9.3 （书中无代码）
# 如果 economics 数据是“宽”的格式时，很容易分别画出两个变量的时间序列图 (左
# 图和中图)，也很容易画出散点图 (右图)。
...
# 变量的时候更方便。由于两个序列取值不在一个数量级上，所以只能看到 unemploy 的变化
# 形式。仔细看会发现在图形的底下还有条线，那就是 uempmed 的趋势线。
...
## 图9.6
# 调整平行坐标图是更好的突出数据模式。为了改进默认的图形 (左上)，我们尝试了
# alpha blending (右上)，jittering (左下) 和同时用这两方法 (右下)。
...
## 图9.8
# faceting 把每个类都放在单独的图内，可以看出类内差异还是相当大的，说明需要增加类的个数。  
...
## 图9.11
# (左) 基本的拟合值 -残差图。(中) 标准化的残差。(右) 点大小与 Cook 距离成比例。
# 只要有全部的数据就可以很容易的修改基本图形。
...
## 图9.12
# 把原数据中的变量加到当前模型里可能会产生新的发现。比如 displ 与 cty 之间
# 似乎是曲线关系，但增加了气缸数量变量 cyl 后，可以看到只要给定了 cyl，displ 与 cty
# 之间其实是呈线性关系的。
full <- basic %+% fortify(mod, mpg)
full + aes(colour = factor(cyl))
full + aes(displ, colour = factor(cyl))
